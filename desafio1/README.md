...
1. O servidor expõe um endpoint `/` na porta 8080. Cada vez que é acessado, retorna uma mensagem JSON e registra a requisição em seus logs. Seu container é baseado em `python:3.10-slim` para ser leve. Por ser o único código que requer uma instalação externa, o `requierements.txt` fica dentro dele.
...
Antes de executar os comandos, certifique-se de estar dentro da pasta `desafio1`.
